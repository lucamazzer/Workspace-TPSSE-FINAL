...
void Board_Buttons_Init(void)
{
	...
	for (idx = 0; idx < (sizeof(gpioButtonBits) / sizeof(io_port_t)); ++idx) {
		/* Set pin direction */
		Chip_GPIO_SetPinDIRInput(LPC_GPIO_PORT, gpioButtonBits[idx].port, gpioButtonBits[idx].pin);		// input
#endif
	}
}
...
uint32_t Buttons_GetStatus(void)
{
	...
	for (idx = 0; idx < (sizeof(gpioButtonBits) / sizeof(io_port_t)); ++idx) {
		if (Chip_GPIO_GetPinState(LPC_GPIO_PORT, gpioButtonBits[idx].port, gpioButtonBits[idx].pin) == 0)
			ret |= 1 << idx;
	}
#endif
	return ret;
